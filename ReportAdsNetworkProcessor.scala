...
object ReportAdsNetworkProcessor extends ReportProcessor  {
	/**
	  * 提供一个目标表名出去
	  */
	override def targetTableName() = AppConfigHelper.REPORT_ADS_NETWORK_TABLE_NAME

	/**
	  * 提供目标表的分区键
	  */
	override def targetTableKeys() = Seq("report_date", "networkmannerid", "networkmannername")
	...
	override def realProcessData(odsDF: DataFrame) = {
		...
		val reportDF: DataFrame = odsDF.sparkSession.sql(
			...
			ReportSQLConstant.reportAdsKpiWithSQL("view_tmp_ods", Seq("networkmannerid", "networkmannername"))
		)
